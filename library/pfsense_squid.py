  ...
  general:
    ...
    suboptions:
      ...
      transparent_proxy:
        ...
        type: bool
      transparent_proxy_active_interface:
...
SQUID_CONFIG_GENERAL_ARGUMENT_SPEC = dict(
  ...
  transparent_proxy_active_interface=dict(required=False, type='str'),
...
SQUID_CONFIG_SYNC_TARGET_ARGUMENT_SPEC = dict(
  ...
  syncport=dict(required=False, type='str'),
  username=dict(required=False, type='str'),
  password=dict(required=False, type='str', no_log=True),
)
...
class PFSenseSquidConfigAuthModule(PFSenseModuleBase):
  ...
  def __init__(self, module, pfsense=None):
    ...
    self.squidauth_elt = pkgs_elt.find('squidauth') if pkgs_elt is not None else None
    self.root_elt = self.squidauth_elt.find('config') if self.squidauth_elt is not None else None
    if self.root_elt is None:
      self.module.fail_json(msg='Unable to find squid XML configuration entry. Are you sure squid is installed?')
...
class PFSenseSquidConfigAntivirusModule(PFSenseModuleBase):
  ...
  def __init__(self, module, pfsense=None):
    ...
    pkgs_elt = self.pfsense.get_element('installedpackages')
    self.squidantivirus_elt = pkgs_elt.find('squidantivirus') if pkgs_elt is not None else None
    self.root_elt = self.squidantivirus_elt.find('config') if self.squidantivirus_elt is not None else None
    if self.root_elt is None:
      self.module.fail_json(msg='Unable to find squid XML configuration entry. Are you sure squid is installed?')
...
class PFSenseSquidConfigCacheModule(PFSenseModuleBase):
  ...
  def __init__(self, module, pfsense=None):
    ...
    pkgs_elt = self.pfsense.get_element('installedpackages')
    self.squid = pkgs_elt.find('squidcache') if pkgs_elt is not None else None
    self.root_elt = self.squid.find('config') if self.squid is not None else None
    if self.root_elt is None:
      self.module.fail_json(msg='Unable to find squid XML configuration entry. Are you sure squid is installed?')
...
class PFSenseSquidConfigGeneralModule(PFSenseModuleBase):
  ...
  def __init__(self, module, pfsense=None):
    ...
    pkgs_elt = self.pfsense.get_element('installedpackages')
    self.squid_elt = pkgs_elt.find('squid') if pkgs_elt is not None else None
    self.root_elt = self.squid_elt.find('config') if self.squid_elt is not None else None
    if self.root_elt is None:
      self.module.fail_json(msg='Unable to find squid XML configuration entry. Are you sure squid is installed?')
  ...
  def _params_to_obj(self):
    ...
    def _set_param_list(target, param):
      if params.get(param) is not None:
        if param in base64_params:
          target[param] = base64.b64encode('\n'.join(params[param]).encode()).decode()
        elif param in ['active_interface', 'transparent_active_interface', 'ssl_active_interface', 'interception_checks', 'google_accounts']:
          target[param] = ','.join(params[param])
...
class PFSenseSquidConfigNacModule(PFSenseModuleBase):
  ...
  def __init__(self, module, pfsense=None):
    ...
    pkgs_elt = self.pfsense.get_element('installedpackages')
    self.squidnac_elt = pkgs_elt.find('squidnac') if pkgs_elt is not None else None
    self.root_elt = self.squidnac_elt.find('config') if self.squidnac_elt is not None else None
    if self.root_elt is None:
      self.module.fail_json(msg='Unable to find squid XML configuration entry. Are you sure squid is installed?')
...
class PFSenseSquidConfigRemoteModule(PFSenseModuleBase):
  ...
  def __init__(self, module, pfsense=None):
    ...
    pkgs_elt = self.pfsense.get_element('installedpackages')
    self.root_elt = pkgs_elt.find('squidremote') if pkgs_elt is not None else None
    if self.root_elt is None:
      self.module.fail_json(msg='Unable to find squid XML configuration entry. Are you sure squid is installed?')
...
class PFSenseSquidConfigSyncModule(PFSenseModuleBase):
  ...
  def __init__(self, module, pfsense=None):
    ...
    pkgs_elt = self.pfsense.get_element('installedpackages')
    self.squidsync_elt = pkgs_elt.find('squidsync') if pkgs_elt is not None else None
    self.root_elt = self.squidsync_elt.find('config') if self.squidsync_elt is not None else None
    if self.root_elt is None:
      self.module.fail_json(msg='Unable to find squid XML configuration entry. Are you sure squid is installed?')
...
class PFSenseSquidConfigUsersModule(PFSenseModuleBase):
  ...
  def __init__(self, module, pfsense=None):
    ...
    self.obj = dict()
    pkgs_elt = self.pfsense.get_element('installedpackages')
    self.root_elt = pkgs_elt.find('squidusers') if pkgs_elt is not None else None
    if self.root_elt is None:
      self.module.fail_json(msg='Unable to find squid XML configuration entry. Are you sure squid is installed?')
